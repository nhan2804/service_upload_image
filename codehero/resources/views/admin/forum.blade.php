...
<br>	
	<div class="row">
		...
		<div class="col-12 col-lg-4 col-sm-12 gutter-6 ">
			...
			{{-- <h3 class="heading_popular">PHỔ BIẾN TRONG NGÀY</h3>
			<ul class="list_popular">
				@foreach($data->skip(0)->take(5) as $key=>$value)
				<li class="desrc_popular_new" title="{{$value->title_blog}}">
	 				<img class="img_popular_new" src="{{asset('public'.$value->img_blog)}}"/>
	 				<div style="margin-left: 8px;">
			 		<a class="link_popular_new " href="{{URL::to('forum/post/'.$value->id_blog)}}">{{$value->title_blog}}</a>
			 		<span class="item_cate" style="color:{{$value->color_cate}};display: block;text-transform: unset;font-size: 0.8rem">{{$value->name}}</span>
			 		</div>
				</li>
				@endforeach
			</ul> --}}
